...
(() => {
  ...
  const stylesheet = `
    ...
    /* no-chrome: author opt-out of the bottom control overlay (slides only). */
    :host([no-chrome]) .overlay { display: none !important; }
  ...
  class DeckStage extends HTMLElement {
    static get observedAttributes() { return ['width', 'height', 'noscale', 'no-rail', 'no-chrome']; }
    ...
    connectedCallback() {
      ...
      // Served/opened as a top-level page (python serve.py, standalone file):
      // presentation only — no thumbnail rail, no bottom control overlay.
      // Inside the editor host (iframe) both stay available.
      try {
        if (window === window.top) {
          this.setAttribute('no-rail', '');
          this.setAttribute('no-chrome', '');
        }
      } catch (_) {}
      this._render();
      this._loadNotes();
    ...
    _onTap(e) {
      // Disabled for this deck: slides are interactive, so a click/tap must
      // never navigate. Keyboard, overlay arrows and the rail cover nav.
      return;
